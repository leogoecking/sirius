...
async function addRecord(data){
  ...
  return new Promise((resolve, reject) => {
    ...
    // Armazena a senha em texto puro, conforme solicitado
...
function render(items){
  itemsBody.innerHTML = '';
  if(!items || items.length===0){ itemsTable.classList.add('hidden'); empty.style.display='block'; return; }
  empty.style.display='none'; itemsTable.classList.remove('hidden');
...
itemsBody.addEventListener('click', async (e)=>{
  ...
  if(action==='delete'){
    ...
    await del(id); showMessage('Registro excluído com sucesso.'); await loadAndRender();
  } else if(action==='save'){
    ...
    const password = row.querySelector('input[data-field="password"]').value; // Senha em texto puro
    if(!isValidMac(mac)) return showMessage('MAC inválido. Verifique o formato.', true);
    try{ await putRecord({ id, mac: normalizeMac(mac), serial, model, password, detectedAt: new Date().toISOString() }); showMessage('Registro atualizado com sucesso.'); await loadAndRender(); }catch(err){ showMessage('Erro ao salvar: '+err.message, true); }
  }
  // Ação 'reveal' removida
});
...
addForm.addEventListener('submit', async (e)=>{
  ...
  const password = passwordInput.value; // Senha em texto puro
  if(!isValidMac(mac)) return showMessage('MAC inválido. Verifique o formato.', true);
  try{
    // Lógica de criptografia removida
    await addRecord({ mac: normalizeMac(mac), serial, model, password });
    macInput.value=''; serialInput.value=''; modelInput.value=''; passwordInput.value='';
    showMessage('Novo registro adicionado com sucesso.'); await loadAndRender();
  }catch(err){ showMessage('Erro ao adicionar registro: '+err.message, true); }
});

exportBtn.addEventListener('click', async ()=>{
  const items = await getAll();
  const blob = new Blob([JSON.stringify(items,null,2)],{type:'application/json'});
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'ont_macs_export.json'; a.click(); URL.revokeObjectURL(url);
...
fileInput.addEventListener('change', async (e)=>{
  ...
  try{
    ...
    if(f.type.includes('json') || text.trim().startsWith('[')){
      ...
      // Importação agora espera o campo 'password' em vez de 'encryptedPassword'
      for(const item of data){ store.put({ ...item, mac: normalizeMac(item.mac), detectedAt: item.detectedAt||new Date().toISOString() }); }
      ...
      // CSV import logic (simplified to only read MAC, Serial, Model from lines)
      ...
      for(const ln of lines){
        ...
        const model = parts[2]||'';
        const password = parts[3]||''; // Adiciona a senha do CSV
        if(isValidMac(mac)){
          try{ await addRecord({ mac: normalizeMac(mac), serial, model, password }); importedCount++; }catch(e){}
        }
      }
      showMessage(`Importação CSV tentada. ${importedCount} registros adicionados/atualizados.`); await loadAndRender();
    }
  }catch(err){ showMessage('Erro ao ler arquivo: '+err.message, true); }
  fileInput.value='';
});

async function loadAndRender(){ const items = await getAll(); render(items); }
